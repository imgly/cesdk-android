...
internal fun Engine.createPendingBlock(
    format: Format,
    customWidth: String,
    customHeight: String,
): DesignBlock {
...
private fun Engine.calculateBlockDimensions(
    ...
    Format.CUSTOM -> {
        ...
        AIImageGenerationUtils.calculateSizeWithAspectRatio(
...
private fun Engine.setPlaceholderFill(block: DesignBlock) {
    ...
    this.block.setColor(
        ...
        value = Color.fromRGBA(r = 0.0F, g = 0.0F, b = 1.0F, a = 0.1F),
    )
    this.block.setFill(block = block, fill = colorFill)
}

internal fun Engine.addImageToBlock(
    block: DesignBlock,
    imageUri: String,
) {
