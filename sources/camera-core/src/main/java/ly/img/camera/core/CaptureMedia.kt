...
import ly.img.camera.core.CaptureMedia.Input

/**
 * An [ActivityResultContract] to start the IMG.LY Camera with the [Input].
 * The output is a [CameraResult] — either [CameraResult.Captures] for standard photo / video /
 * mixed sessions, or [CameraResult.Reaction] for reaction sessions.
 */
open class CaptureMedia : ActivityResultContract<Input, CameraResult?>() {
    @CallSuper
     ...
     *
     * @throws IllegalArgumentException if [cameraMode] does not support
     * `cameraConfiguration.captureType` (see [CameraMode.supports]).
     */
    open class Input(
        ...
        init {
            require(cameraMode.supports(cameraConfiguration.captureType)) {
                "CameraMode ${cameraMode::class.simpleName} does not support " +
