...
fun createEditorPagesState(
    ...
    val pages = engine.scene.getPages().map { pageBlock ->
...
fun EditorPagesState.copy(
    ...
    val newPages = engine.scene.getPages().map { pageBlock ->
    ...
    val newSelectedPage = selectedPage ?: run {
...
private fun createEditorPagesState(
    ...
    val pageAspectRatio = pages.firstOrNull()?.let {
